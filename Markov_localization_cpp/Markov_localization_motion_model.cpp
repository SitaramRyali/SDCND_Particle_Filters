...
float motion_model(float pseudo_position, float movement, vector<float> priors,
	...
	//each pseudo_position is current x value;
	// YOUR CODE HERE
	int diff_pos;
	float prob_x_i;
	float trans_prob;
	...
	{
		diff_pos = pseudo_position - i;
		trans_prob = Helpers::normpdf(diff_pos, movement, control_stdev)* (2 * M_PI)/sqrt(2 * M_PI);
		bel_xt = priors[i];
		prob_x_i = trans_prob * bel_xt;
		position_prob += prob_x_i;
	}
